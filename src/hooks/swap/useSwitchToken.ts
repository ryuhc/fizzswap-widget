...
export function useSwitchToken({
  ...
  const refreshEstimate = useCallback(
    (_inputToken: ITokenItem) => {
      const newInput = isMaxInput
        ? toReadableBN(
            balances[_inputToken.address] ?? 0n,
            _inputToken.decimal,
            _inputToken.decimal
          )
        : typedField === 0
          ...
          : outputValue
      const fixedNewInput =
        isMaxInput && isSameAddress(_inputToken.address, nativeAddress)
          ? subBN(newInput, MIN_AMOUNT_FOR_FEE)
          : newInput

      onInput(
        new BN(fixedNewInput).comparedTo('0') === -1 ? '0' : fixedNewInput
      )
    },
    [
      isMaxInput,
