...
describe('test timer on a2a modal. default timeout is 5 minute', () => {
  it('check initial state', () => {
    ...
    expect(result.current).toEqual({ remain: { minutes: 5, seconds: 0 }, isFinished: false })
  })

  it('remain 4m50s after 10 seconds', () => {
    const { result, rerender } = renderHook(useA2AModalActions, { initialProps })
    ...
    expect(result.current).toEqual({ remain: { minutes: 4, seconds: 50 }, isFinished: false })
  })
  ...
  it('isFinished to false and call props.onClose after timeout', () => {
    ...
    expect(result.current).toEqual({ remain: { minutes: 0, seconds: 0 }, isFinished: true })
