...
export function dclean(_v: string | number) {
  // 불필요한 0 제거
  const v = String(_v)
...
export function dprec(_v: any, d: number, force = false) {
  // 강제로 소수점 이하 d자리 출력
...
export function mulBNWithDecimal(
  a: Value | bigint,
  b: Value | bigint,
  decimal = 0,
  withComma = false
...
export function divBN(a: Value | bigint, b: Value | bigint, withComma = false) {
  const res =
    Number.isNaN(a) || Number(a) === 0 || Number.isNaN(b) || Number(b) === 0
      ? '0'
      : new BN(String(a || '0')).dividedBy(String(b || '0')).toString()
  return withComma ? addComma(res) : res
}
export function divBNWithDecimal(
  a: Value | bigint,
  b: Value | bigint,
  decimal = 0,
  withComma = false
  ...
  const res =
    Number.isNaN(a) || Number(a) === 0 || Number.isNaN(b) || Number(b) === 0
      ...
      : new BN(
          new BN(String(a || '0'))
            .dividedBy(String(b || '0'))
            .decimalPlaces(decimal, 1)
        ).toString()
  return withComma ? addComma(res) : res
}
export function toReadableBN(
  a: Value | bigint,
  decimal: number,
  precision = 6,
  withComma = false
...
export function toWritableUnit(a: Value | bigint, decimal: number) {
  return a === '' || a === '0'
    ? '0'
    : new BN(
        new BN(String(a || '0')).multipliedBy(10 ** decimal).decimalPlaces(0, 1)
      ).toString()
}
...
export function maxBN(
  a: string | BigNumber | bigint | number,
  b: string | BigNumber | bigint | number
) {
  return new BN(typeof a === 'bigint' ? String(a) : a).comparedTo(
    typeof b === 'bigint' ? String(b) : b
  ) === 1
    ? a
    : b
}
