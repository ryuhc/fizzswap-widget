...
export function FormTokenBalance({ token, balance, isMax, onClickMax, hideMax }: IProps) {
  const { t } = useTranslationSimplify()

  return (
    <StyledFormTokenBalance>
      <StyledTitle>{t('General.Balance')}</StyledTitle>
      <StyledValue data-testid="form-token-balance-value">{token && balance ? toReadableBN(balance ?? 0n, token?.decimal, 6, true) : 0}</StyledValue>
      {token && !hideMax ? <StyledMaxButton data-testid="form-token-balance-max" ismax={isMax.toString()} onClick={() => onClickMax()}>max</StyledMaxButton> : null}
    </StyledFormTokenBalance>
  )
}
