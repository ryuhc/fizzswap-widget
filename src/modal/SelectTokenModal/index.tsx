...
export function SelectTokenModal({
  ...
  const [searchedToken, setSearchedToken] = useState<
    (ITokenItem & { balance: bigint }) | null
  >(null)
  ...
  const handleInputKeyword = useCallback(
    (typedValue: string) => {
      ...
      if (
        typedValue.length > 1 ||
        (typedValue.length === 0 && keyword.length > 0)
      ) {
        setTimeout(() => {
          remove()
          refetch()
          ...
          <StyledTokenTable>
            <StyledTokenTableHeader>
              ...
              <StyledTokenTableHeaderCol className="balance">
                {t('General.Balance')} /{' '}
                {t('General.SelectTokenEstimatedValue')}
              </StyledTokenTableHeaderCol>
            </StyledTokenTableHeader>
            ...
            {searchedToken ? (
              <StyledTokenTableScrollable>
                <StyledTokenTableRow
                  ...
                  onClick={() => handleSelect({ ...searchedToken, price: '0' })}
                >
                  <StyledTokenTableRowCol className="token">
                    ...
                    <StyledTokenTableRowColToken>
                      ...
                      >
                        {pickTokenName(searchedToken, i18n.language)}
                      </Paragraph>
                  ...
                  <StyledTokenTableRowCol className="balance">
                    <Paragraph
                      weight={700}
                      style={{ wordBreak: 'break-all', paddingLeft: '5px' }}
                    >
                      {searchedToken.balance === 0n
                        ? 0
                        : toReadableBN(
                            searchedToken.balance,
                            searchedToken.decimal,
                            6,
                            true
                          )}
                    </Paragraph>
                ...
                >
                  ...
                  )}
                  {
                    renderList.map((token: ITokenItem) => {
                      ...
                      const readableBalance = toReadableBN(
                        balance,
                        token.decimal,
                        token.decimal
                      )
                      ...
                      return (
                        !(
                          hideNative && isNativeToken(token.address, chainId)
                        ) && (
                          <StyledTokenTableRow
                            key={token.address}
                            selected={isSameAddress(
                          ...
                          >
                            <StyledTokenTableRowCol className="token">
                              ...
                              <StyledTokenTableRowColToken>
                                <Paragraph
                                  ...
                                  style={{ marginBottom: '3px' }}
                                >
                                  {pickTokenName(token, i18n.language)}
                                </Paragraph>
                                <Paragraph size={12} color="gray">
                                  {token.symbol}
                                </Paragraph>
                            ...
                            <StyledTokenTableRowCol className="balance">
                              ...
                              >
                                {balance === 0n
                                  ? 0
                                  : addComma(dprec(readableBalance, 6))}
                              </Paragraph>
                              <Paragraph size={12}>
                                ${' '}
                                {dprec(
